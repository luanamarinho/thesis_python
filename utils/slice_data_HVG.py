...
def slice_data_HVG(expr_data, n_top_genes = 0.1):
    """
    ...
    - n_top_genes: Float indicating the percentage of the top HVG genes to be retrieved.

    Returns:
    - A sliced expression data matrix with only the n_top HVG.
    """
    adata = AnnData(X = expr_data)
    sc.pp.highly_variable_genes(adata, n_top_genes=int(n_top_genes * adata.shape[1]), flavor='seurat_v3')
