...
def trustworthiness_chunks_multik(dist_X, dist_embedded, k = [30, 300]):
    """
    Parameters:
    ...
    #inverted_index[ordered_indices[:, np.newaxis], ind_X] = np.arange(n_cols)
    ...
    #t = []
    t = [t_chunk(inverted_index, ordered_indices, ind_X_embedded, k_value) for k_value in k]
    #for k_value in k:
      #t.append(t_chunk(inverted_index, ordered_indices, ind_X_embedded, k_value))
    return t

def t_chunk(inverted_index, ordered_indices, ind_X_embedded, k_value):
      if k_value > inverted_index.shape[1]:
        raise ValueError(f"k={k_value} is greater than the number of columns in inverted_index.")
      ranks = (
        inverted_index[ordered_indices[:-1, np.newaxis], ind_X_embedded[:,:k_value]] - k_value
      )
      t_out = np.sum(ranks[ranks > 0])
      return t_out